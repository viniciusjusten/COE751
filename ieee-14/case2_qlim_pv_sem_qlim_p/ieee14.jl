...
println("\n============================================================")
println("Sumário por Barra: IEEE 14 Bus - Caso 2 (QLIM em PV, CREM)")
println("============================================================")
...
for (i, bus) in enumerate(power_flow_case.buses)
    ...
    if bus.type == COE751.Bus_type.Slack
        @printf("  Especificado: V = %.3f pu  |  θ = %.1f° (referência)\n",
            bus.voltage_magnitude, rad2deg(bus.voltage_angle))
        @printf("  Calculado:    V = %.3f pu  |  θ = %7.3f°\n", v[i], a[i])
        @printf("                P_inj = %8.3f MW   |  Q_inj = %8.3f Mvar  (injeção líquida)\n", P[i], Q[i])
        @printf("                Pg    = %8.3f MW   |  Qg    = %8.3f Mvar  (geração)\n", Pg_calc, Qg_calc)

    elseif bus.type == COE751.Bus_type.PV
        # P totalmente especificado (Pg e Pl); Q: apenas Ql, Qg calculado
        P_inj_spec = Pg_spec - Pl_MW
        @printf("  Especificado: V = %.3f pu\n", bus.voltage_magnitude)
        @printf("                P: Pg = %8.3f MW   (geração)  |  Pl = %8.3f MW   (carga)  |  P_inj = %8.3f MW\n",
            Pg_spec, Pl_MW, P_inj_spec)
        @printf("                Q:                              |  Ql = %8.3f Mvar (carga)  [Qg calculado]\n",
            Ql_Mvar)
        if has_qlim
            ...
            println("                Qlim = [$qmin_s, $qmax_s] Mvar")
        end
        @printf("  Calculado:    V = %.3f pu  |  θ = %7.3f°\n", v[i], a[i])
        @printf("                P_inj = %8.3f MW   |  Q_inj = %8.3f Mvar  (injeção líquida)\n", P[i], Q[i])
        @printf("                Pg    = %8.3f MW   |  Qg    = %8.3f Mvar  (geração)\n", Pg_calc, Qg_calc)
    ...
    elseif bus.type == COE751.Bus_type.PQ
        ...
        @printf("  Especificado: P: Pg = %8.3f MW   (geração)  |  Pl = %8.3f MW   (carga)  |  P_inj = %8.3f MW\n",
            Pg_spec, Pl_MW, P_inj_spec)
        @printf("                Q: Qg = %8.3f Mvar (geração)  |  Ql = %8.3f Mvar (carga)  |  Q_inj = %8.3f Mvar\n",
            Qg_spec, Ql_Mvar, Q_inj_spec)
        if has_shunt
            @printf("                Shunt capacitivo: B = %.3f pu = %.1f Mvar (a V = 1 pu)\n",
    ...
    elseif bus.type == COE751.Bus_type.PQV
        # P e Q totalmente especificados; V fixo por controle remoto
        P_inj_spec = Pg_spec - Pl_MW
        Q_inj_spec = Qg_spec - Ql_Mvar
        @printf("  Especificado: V = %.3f pu (tensão de referência)\n", bus.voltage_magnitude)
        @printf("                P: Pg = %8.3f MW   (geração)  |  Pl = %8.3f MW   (carga)  |  P_inj = %8.3f MW\n",
            Pg_spec, Pl_MW, P_inj_spec)
        @printf("                Q: Qg = %8.3f Mvar (geração)  |  Ql = %8.3f Mvar (carga)  |  Q_inj = %8.3f Mvar\n",
            Qg_spec, Ql_Mvar, Q_inj_spec)
